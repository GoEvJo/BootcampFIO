/*
Tipos de datos
Un estudiante de programación intentó realizar declaraciones de
variables con sus respectivos tipos en Go pero tuvo varias dudas
mientras lo hacía. A partir de esto, nos brindó su código y pidió la ayuda
de un desarrollador experimentado que pueda:
● Verificar su código y realizar las correcciones necesarias.
*/

package main

func main() {
	var apellido string = "Gómez"

	//var edad int = "35"
	var edad int = 35

	/* boolean := "false";
	[!] preguntar si se puede usar 'boolean' como nombre */
	buleano := false

	//var sueldo string = 45857.90
	var sueldo string = "45857.90"

	var nombre string = "Julián"
}
